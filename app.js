import { calculatePenaltyView, evaluateConditionalSpeed, parseSpeedValue } from './logic.js';
...
function resolveRoadLimit(match) {
  ...
  if (!parsed && directionalKey && tags[directionalKey]) {
    parsed = parseSpeedValue(tags[directionalKey]);
  ...
  if (!parsed && tags['maxspeed:motorcar']) {
    parsed = parseSpeedValue(tags['maxspeed:motorcar']);
    source = 'OSM-Limit für Pkw';
  }

  if (!parsed && tags.maxspeed) {
    parsed = parseSpeedValue(tags.maxspeed);
    source = parsed?.source || 'Explizites OSM-Tempolimit';
    if (parsed?.source) confidence = 'medium';
  }

  if (!parsed && tags['zone:maxspeed']) {
    parsed = parseSpeedValue(tags['zone:maxspeed']);
    if (parsed) {
  ...
  if (!parsed) {
    const defaultParsed = parseSpeedValue(defaultToken);
    if (defaultParsed) {
...
function render() {
  ...
  } else if (limitInfo.confidence === 'low') {
    els.delta.textContent = `${delta} km/h`;
    els.fine.textContent = '—';
    els.points.textContent = '—';
    els.ban.textContent = '—';
    els.fineNote.textContent = 'Tempolimit nur aus Straßentyp oder Umgebung abgeleitet. Keine belastbare Bußgeldprognose; Beschilderung prüfen.';
  } else {
    els.delta.textContent = `${delta} km/h`;
